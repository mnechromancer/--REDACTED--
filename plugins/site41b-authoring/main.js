...
`);await this.app.workspace.getLeaf().openFile(c),new a.Notice(`Created ${c.basename} \u2014 run build:corpus to validate.`)}catch(c){new a.Notice(`Could not create file: ${c.message}`)}}},T=class extends a.PluginSettingTab{constructor(s,e){super(s,e),this.plugin=e}display(){let{containerEl:s}=this;s.empty(),s.createEl("h2",{text:"Site-41B Authoring"}),new a.Setting(s).setName("Anthropic API key").setDesc("Required for the Wiki Generator. Get one at console.anthropic.com.").addText(e=>e.setPlaceholder("sk-ant-\u2026").setValue(this.plugin.settings.anthropicApiKey).onChange(async t=>{this.plugin.settings.anthropicApiKey=t.trim(),await this.plugin.saveSettings()})),new a.Setting(s).setName("Model").setDesc("Claude model ID to use for wiki generation.").addText(e=>e.setPlaceholder("claude-sonnet-4-6").setValue(this.plugin.settings.model).onChange(async t=>{this.plugin.settings.model=t.trim()||S.model,await this.plugin.saveSettings()}))}},B=class extends a.Plugin{constructor(){super(...arguments);this.settings=S}async onload(){await this.loadSettings(),this.registerView(m,e=>new v(e,this)),this.registerView(w,e=>new k(e,this)),this.addCommand({id:"open-build-panel",name:"Open build validator",callback:()=>this.openView(m)}),this.addCommand({id:"open-wiki-generator",name:"Open wiki generator",callback:()=>this.openView(w)}),this.addCommand({id:"run-build-corpus",name:"Run build:corpus",callback:()=>this.quickBuild()}),this.addCommand({id:"run-winnable-test",name:"Run winnable test",callback:()=>this.quickTest()}),this.addRibbonIcon("check-circle","Site-41B: build validator",()=>this.openView(m)),this.addRibbonIcon("sparkles","Site-41B: wiki generator",()=>this.openView(w)),this.addSettingTab(new T(this.app,this))}async openView(e){let t=this.app.workspace.getLeavesOfType(e);if(t.length){this.app.workspace.revealLeaf(t[0]);return}let i=this.app.workspace.getRightLeaf(!1);i&&(await i.setViewState({type:e,active:!0}),this.app.workspace.revealLeaf(i))}async quickBuild(){let e=new a.Notice("Running build:corpus\u2026",0),t=await E(f(this.app),"npm run build:corpus");e.hide(),t.success?new a.Notice("\u2713 build:corpus passed"):(new a.Notice("\u2717 build:corpus failed \u2014 see Build panel for details"),this.openView(m))}async quickTest(){let e=new a.Notice("Running winnable test\u2026",0),t=await E(f(this.app),"npx vitest run src/lib/__tests__/real-corpus-winnable.test.ts");e.hide(),t.success?new a.Notice("\u2713 Winnable test passed"):(new a.Notice("\u2717 Winnable test failed \u2014 see Build panel for details"),this.openView(m))}async loadSettings(){this.settings=Object.assign({},S,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}};0&&(module.exports={BuildView,WikiView});
