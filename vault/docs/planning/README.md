...
> **⚠ Second pivot (2026-06-17): the v2 reset.** Current direction. Read **`reset_amber_v2.md`** (the plan), then **`handoff_reset_build.md`** (the Phase-1 file-by-file build), then `amber_build_decisions.md` §"v2 reset" (the locked §6 A–D answers). Phase-1 playtest produced **`design_note_forged_citations.md`** — the player *forges* citations from highlighted spans (a Phase-2/3 upgrade to the citation verb); read it before the CLL/citation UI rebuild. The reset keeps the AMBER/Quippy split and the no-Quippy win but **replaces the redaction primitive** (sentence-rewrite mutation sets → single redacted word recovered by citing where the corpus grounds it), cuts clearance for pure-graph reachability, and resets onboarding. The 2026-06-13 re-frame (`reframe_amber_quippy.md`) is the **prior** north star — still readable, superseded-in-part by the reset.
>
