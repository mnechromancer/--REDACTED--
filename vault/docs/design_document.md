...
2. **Choose your tool.** To unredact a slot you switch into a mode:
   - **AMBER (the hard, safe way):** use AMBER's manual unredaction tooling — cross-reference lookup, concordance-by-hand, citation tracing — to assemble the case for a value from the *other files*, then commit it. The redaction is grounded in multiple sources, so once you know enough the fill is an earned "a-ha," not a guess. **PENDING [R§6.2]:** the concrete AMBER tooling mechanics are unspecced and gate the build; §5.3 states the target and the constraints, not the final interaction.
...
**PENDING [R§6.6]:** how much of the CLI ships in the prototype — full keyboard-driven traversal now, or CLI file-rendering with the current interaction model first and CLI traversal next — is a human decision affecting how much of the existing Svelte UI is rebuilt vs. restyled. The target architecture is in `technical_document.md` §7; this section states the design intent, not the prototype cut.

### 5.3 Manual unredaction in AMBER (the honest tooling)  — **PENDING [R§6.2]**
This is the single biggest open design surface and it **gates the build.** The *intent* is fixed; the *mechanics* are not.

Intent: unredacting a slot in AMBER is an **evidentiary process**, not a dropdown. The player uses AMBER's analysis tools to build and commit the case for a value:
- **cross-reference lookup** — every other file where this slot's concept appears (the seed of this is the prior design's hover panel, now promoted into AMBER tooling — `conceptClues` in the engine);
- **concordance-by-hand** — AMBER's legitimate cross-reference index (the **Concordance**: [R§6.1], adopted — the Concordance is *AMBER's honest tool*, which Quippy imitates and masks; it is **not** the entity);
- **citation / frequency tracing** — following how a concept is described across the corpus to triangulate the slot.

The player assembles enough to *justify* a candidate, then commits it. Because each redaction is grounded in multiple files (§7), a sufficiently-read player can do this; a player who hasn't read enough cannot, and reaches for Quippy. **What is unspecced:** the exact interaction — is it "find N corroborating citations to unlock the commit," a free-form justification the engine checks against the truth, a tiered hint economy, something else? This determines the core verb and must be answered by the human before the mechanic is built. Until then, treat AMBER unredaction as "the bounded typed-slot insertion of §5.4, reached through a manual evidence process whose shape is TBD."
...
- **The two unspecced mechanics gate everything** ([R§6.2] AMBER tooling, [R§6.6] CLI scope). Until the human resolves them, the loop's core verb (manual unredaction) and the prototype's UI cut are open. The docs are written so design and spec are ready the moment those land.
