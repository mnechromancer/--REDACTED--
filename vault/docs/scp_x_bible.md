...
- [ ] **PENDING [R§6.3 enforcement] — the no-Quippy ending's exact gate** (hard / tolerance / spectrum; §5.3). Tracking via provenance is settled; the threshold is not. *(Still open — a tuning decision, not authoring.)*
- [x] ~~**Author `SCP-41B-000` (Quippy's self-file) last.**~~ **DONE (2026-06-14).** The real self-file (`vault/entries/SCP-41B-000.md`) replaces the play-test placeholder: a full Keter dossier, `entity_self: true`, five anchors threading the five entity keys at their high-tier index-2 readings, excluded from the restoration target per §5.4. Build/check/test green. Co-carriers 018/020/024 for `concordance-program`/`halloran-marginalia`/`the-transfer`/`record-reality-coupling` are still unwritten — 000 is the sole authored carrier of each (length 3, truth index 2, alignment-ready); match the registry index meanings when authoring them.
- [x] ~~**Quippy voice pass** (§2.2, §3, §4).~~ **DONE (2026-06-14).** `QUIPPY_GREETING` + `quippyFillLine` in `src/lib/quippy.svelte.ts` rewritten against the casting rule: bright/clerical at low, the first quiet *we* mid-sentence at mid, proprietary "ours/re-shelving" at high, the act dropped post-breach. Marsh-calm throughout; the in-panel lines in `QuippyPanel.svelte` aligned.
- [x] ~~**AMBER voice pass** (§2.1).~~ **DONE (2026-06-14).** `AmberLookup.svelte` / `AmberTerminal.svelte` reject + log lines given the Quality-Approval error-code coldness (`E10/E21/E30 REJECT … commit aborted`, `COMMIT OK …`); the clinical register is the contrast that makes Quippy legible.
- [x] ~~**Halloran-marginalia recast** (§6).~~ **DONE (2026-06-14).** Halloran's marginalia across the trio (003 → 002 → 001) now read as one predecessor degrading from method to warning to surrender — a warning about the *tool*, legible only once the reader knows which tool Halloran trusted. The original answer-clue ("count the copies, not the years") is preserved inside that arc.
