...
> **Currency note (updated 2026-07-04, v3):** the **methodology in this doc is current** (the pipeline, personas, severity tiers, least-privilege context scoping). Its **design examples are two pivots stale** — they predate the v2 single-word primitive and the v3 receiving-site frame. For any *design* specifics the authority is `spec_game.md`; for the pivots' history, `planning/amber_build_decisions.md` and `archive/`. Do not quote this doc's mechanics examples; do follow its process.

How this game gets built and authored by agents. Method aligned to the five CCA-F (Claude Certified Architect – Foundations) exam domains; every pattern named below is a pattern from that blueprint applied to this repo's actual work. Companion docs: `spec_game.md` (what to build), `site_41b.md` (what to write), `handoff.md` (context routing).
